...
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | 是 | Clerk 前端发布 key，用于登录组件和浏览器会话。 |
| `CLERK_SECRET_KEY` | 是 | Clerk 服务端 secret key，用于 Agent 路由和管理页鉴权。 |
| `NEXT_PUBLIC_CLERK_SIGN_IN_URL` | 否 | 登录页路径，默认可使用 `/sign-in`。 |
| `NEXT_PUBLIC_CLERK_SIGN_UP_URL` | 否 | 注册页路径，默认可使用 `/sign-up`。 |
| `NEXT_PUBLIC_CLERK_SIGN_IN_FALLBACK_REDIRECT_URL` | 否 | 登录成功后的兜底跳转路径。 |
| `NEXT_PUBLIC_CLERK_SIGN_UP_FALLBACK_REDIRECT_URL` | 否 | 注册成功后的兜底跳转路径。 |
| `SUPABASE_URL` | 是 | 用量和权限数据库的 Supabase 项目 URL。 |
| `SUPABASE_SERVICE_ROLE_KEY` | 是 | 调用用量/权限 RPC 的 Supabase service role key。 |
...
`Clerk is not configured` 表示运行环境缺少 `CLERK_SECRET_KEY` 或
`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`。本地 `.env.local` 不会随代码部署；
上线前请在 EdgeOne Makers 控制台或 `edgeone makers env set` 中配置同名变量。
...
复制后请填入真实的 AI 网关、Clerk 和 Supabase 配置。
